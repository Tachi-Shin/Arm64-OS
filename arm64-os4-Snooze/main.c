...
int Timer(void)
{
    ...
    for (TaskIdType task = 0; task < NUMBER_OF_TASKS; task++) {
...
int InterruptHandler(void)
{
    ...
    if (irq == GEN_TIMER_INTID) {             // タイマIRQ？
        preempt = Timer();                    // 次回比較値設定など。プリエンプトするなら1を返す
    }

    WriteGICC32(GICC_EOIR, iar);             // EOI
...
void main(void) {
    clearbss();
    SetVectorTable();     // VBAR_EL1 に vector_table を登録
    SetupGIC();
    StartTimer();
